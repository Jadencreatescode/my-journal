...
def _validated_request(value: object) -> dict[str, Any]:
    ...
    if operation not in _TOOL_HANDLERS:
...
def _prepare_environment(request: dict[str, Any]) -> None:
    os.environ["HERMES_HOME"] = windows_to_wsl_path(request["windows_home"])
    os.environ["MY_JOURNAL_ROOT"] = windows_to_wsl_path(request["windows_journal_root"])
...
def dispatch(value: object, *, plugin=None) -> dict[str, Any]:
    ...
    handler = getattr(loaded.tools, _TOOL_HANDLERS[request["operation"]])
